...
siteslist = list()
with open('../database/sites/sites2.txt') as f:
    for line in f:
...
database = "../database/requestdata2/crawl-data.sqlite"
connt = create_connection(database)
cursor = connt.cursor()
checkapi = checkapi.checkapi("../database/requestdata2/crawl-data.sqlite", "../database/apichecked/result2.sqlite", startid, stopid)
checkapi.run()
###################################